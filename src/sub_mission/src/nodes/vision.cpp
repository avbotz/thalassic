   ...
   public:
    ...
    BT::NodeStatus onStart() override {
        ...
        if (!validDistanceValue(vision_distance_m_) || pass_distance_ < 0.0 || min_transit_distance_ < 0.0 ||
            ...
            min_progress_m_ <= 0.0 || progress_timeout_msec_ <= 0 || max_planned_distance_ < 0.0) {
            RCLCPP_ERROR(logger_,
    ...
    BT::NodeStatus onRunning() override {
        ...
        if (forward_progress >= completion_distance_) {
            RCLCPP_INFO(logger_,
                        "ForwardFixedTransit: completed %.2fm confirmed-gap pass (planned %.2fm).",
                        forward_progress, planned_distance_);
            stopVelocity();
            return BT::NodeStatus::SUCCESS;
        }
        if (forward_progress >= last_progress_travel_ + min_progress_m_) {
            last_progress_travel_ = forward_progress;
            progress_deadline_ = SteadyClock::now() + std::chrono::milliseconds(progress_timeout_msec_);
        } else if (SteadyClock::now() >= progress_deadline_) {
            RCLCPP_WARN(logger_,
                        "ForwardFixedTransit stopped after %.2fs without %.2fm forward progress "
                        "(forward=%.2fm).",
                        progress_timeout_msec_ / 1000.0, min_progress_m_, forward_progress);
            stopVelocity();
            return BT::NodeStatus::FAILURE;
        }
