...
void SimPressureToDepth::pressure_callback(const sensor_msgs::msg::FluidPressure::SharedPtr msg) {
    // Stonefish publishes absolute pressure; remove atmospheric pressure so
    // the surface maps to zero depth as it does on the vehicle.
    const double depth = std::max(0.0, (msg->fluid_pressure - surface_pressure_pa_) /
                                           (water_density_kg_m3_ * gravity_m_s2_));
