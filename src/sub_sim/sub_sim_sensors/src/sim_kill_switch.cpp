...
SimKillSwitch::SimKillSwitch() : Node("sim_kill_switch") {
    ...
    sim_sub_ = this->create_subscription<std_msgs::msg::Bool>(
        "sim/kill_switch", qos, std::bind(&SimKillSwitch::sim_callback, this, _1));
    ...
    startup_timer_ = this->create_wall_timer(
        std::chrono::duration<double>(off_delay),
        std::bind(&SimKillSwitch::release, this));
}
...
void SimKillSwitch::release() {
    ...
    RCLCPP_INFO(this->get_logger(),
                "kill switch OFF (sub armed); now forwarding sim/kill_switch");
}
