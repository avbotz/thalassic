 ...
 * mixer.c
 *
 * Ported verbatim from Nautical-Private src/mixer.c.
 *
...
void mec_mix(struct mec_force_setpoint *force_sp, struct mec_torque_setpoint *torque_sp, float mix[8][6], float power,
    ...
    float force_setpoints[] = {
        force_sp->forward, force_sp->right, force_sp->down, torque_sp->roll, torque_sp->pitch, torque_sp->yaw,
    };
    ...
    for (int thruster = 0; thruster < NUM_THRUSTERS; thruster++) {
        ...
        for (int dof = 0; dof < NUM_DOF; dof++) {
            thruster_outputs[thruster] += force_setpoints[dof] * mix[thruster][dof];
        }
    }
