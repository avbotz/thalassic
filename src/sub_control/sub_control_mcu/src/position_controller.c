...
void position_controller_update(struct position_controller *ctrl, struct mec_vehicle_position *pos,
    ...
    position_ned_to_body(&error, &ned_error, att);

    float max_speed = 1.;

    output->forward_m_s = normalize(pid_calculate(&ctrl->pid[0], error.forward, dt), -max_speed, max_speed);

    output->right_m_s = normalize(pid_calculate(&ctrl->pid[1], error.right, dt), -max_speed, max_speed);
